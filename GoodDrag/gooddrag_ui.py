...
from utils.ui_utils import (
    ...
    save_intermediate_images, create_video, keypoint_detection
)
...
def create_real_image_editing_ui():
    with gr.Row():
        ...
        with gr.Column():
            ...
            with gr.Row():
                keypoint_detection_button = gr.Button("Keypoint Detection") ## 내가 추가한 버튼. 이 버튼을 누르면 keypoint detection이 실행됨.
    ...
    return canvas, train_lora_button, lora_path, lora_status_bar, input_image, keypoint_detection_button, undo_button, save_button, data_dir, \
           output_image, run_button, clear_all_button, show_points, result_save_path, save_result #keypoint_detection_button 추가
...
## keypoint_detection을 위한 함수 추가
def attach_keypoint_detection_event(keypoint_detection_button, input_image, prompt, selected_points):
    keypoint_detection_button.click(
        keypoint_detection,
...
def main():
    with gr.Blocks() as demo:
        ...
        canvas, train_lora_button, lora_path, lora_status_bar, input_image, keypoint_detection_button, undo_button, save_button, data_dir, \
        output_image, run_button, clear_all_button, show_points, result_save_path, \
        save_result = create_real_image_editing_ui() ## keypoint_detection_button 추가
        ...
        attach_keypoint_detection_event(keypoint_detection_button, input_image, prompt, selected_points) ## keypoint_detection을 위한 함수 추가
