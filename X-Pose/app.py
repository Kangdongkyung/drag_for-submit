...
# 내가 추가한 함수
# 디버그 용. print하는 함수.
def print_keypoint_coordinates(tgt, keypoint_text_prompt, image_size=(512, 512)):
    if 'keypoints' in tgt:
        num_kpts = len(keypoint_text_prompt)
        for ann in tgt['keypoints']:
            kp = np.array(ann.cpu())
            Z = kp[:num_kpts * 2]
            x = Z[0::2] * image_size[0]
            y = Z[1::2] * image_size[1]
            for idx in range(num_kpts):
                print(f"Keypoint {idx + 1}: ({x[idx]}, {y[idx]})")

# 이 함수는 이미지를 출력하지 않고, 각 keypoint의 정보를 출력합니다.
# tgt는 출력할 대상 이미지와 관련된 정보를 담고 있는 dictionary입니다.
# keypoint_text_prompt는 각 keypoint에 대한 설명을 담고 있는 리스트입니다.
...
def plot_on_image(image_pil, tgt, keypoint_skeleton, keypoint_text_prompt):
    ...
    if 'keypoints' in tgt:
        ...
        for idx, ann in enumerate(tgt['keypoints']):
            ...
            for i in range(num_kpts):
                c_kpt = color_kpt[i % len(color_kpt)]
    ...
    ## Image.ANTIALIAS 는 pillow == 8.3.0 이상부터 deprecated 되었음. Image.LANCZOS로 대체 
    image_with_predict = image_with_predict.resize((512, 512), Image.LANCZOS)
...
### 내가 추가한 함수
def load_model_for_gooddrag(model_config_path, model_checkpoint_path, args, cpu_only=False):
    args = Config.fromfile(model_config_path)
    args.device = "cuda" if not cpu_only else "cpu"
    model = build_model(args)
    checkpoint = torch.load(model_checkpoint_path, map_location="cpu")
...
config_file = os.path.join(script_dir, 'config_model', 'UniPose_SwinT.py')  # Change the path of the model config file
checkpoint_path = os.path.join(script_dir, 'weights', 'unipose_swint.pth')  # Change the path of the model
            ...
            with gr.Column():
                ...
                with gr.Accordion("Advanced options", open=False):
                    box_threshold = gr.Slider(
                        label="Box Threshold", minimum=0.0, maximum=1.0, value=0.05, step=0.001  ## keypoint의 원활한 검출을 위하여 0.05로 내림.
                    )
